...
package ELF is
    ...
    type Object_File_Type is
       (NONE, REL, EXEC, DYN, CORE, LOOS, HIOS, LOPROC, HIPROC);
    for Object_File_Type use
       (NONE => 0, REL => 1, EXEC => 2, DYN => 3, CORE => 4, LOOS => 16#FE00#,
        HIOS => 16#FEFF#, LOPROC => 16#FF00#, HIPROC => 16#FFFF#);
    ...
    for Segment_Type use
        (PT_NULL => 0, PT_LOAD => 1, PT_DYNAMIC => 2, PT_INTERP => 3, PT_NOTE => 4, PT_SHLIB => 5, PT_PHDR => 6, PT_LOPROC => 16#70000000#, PT_HIPROC => 16#7FFFFFFF#);
    
    type Segment_Flags is
        (PF_X, PF_W, PF_R);

...
package body VGA.GTF is
   ...
   function Get_Configuration (H_PIXELS : Pixel_Count; V_LINES : Scan_Line_Count) return VGA_Configuration
   is
      ...
      function To_Character_Count (pixel : Pixel_Count) return Character_Count is ((Character_Count (Pixel_Round_Divide (pixel, CELL_GRAN))));
     ...
     -- 7. Find the ideal horizontal period from the blanking duty cycle equation:
      test : Float := Util.sqrt(((100.0 - C)**2) + (4.0* M * Float (TOTAL_ACTIVE_PIXELS + RIGHT_MARGIN + LEFT_MARGIN) / PIXEL_FREQ / 10.0));
   ...
   --    -- 9. Find the number of pixels in the blanking time to the nearest double character cell:
      H_BLANK : Pixel_Count := Pixel_Count (Float'Rounding (Float (TOTAL_ACTIVE_PIXELS) * IDEAL_DUTY_CYCLE / (100.0 - IDEAL_DUTY_CYCLE) / (2.0 * Float (CELL_GRAN))))* Pixel_Count (2*CELL_GRAN);
   ...
   --    -- 16. Find the number of lines in V sync + back porch:
      V_SYNC_BP : Scan_Line_Count := Scan_Line_Count (Float'Rounding (MIN_VSYNC_BP * H_FREQ / 1000.0));
   ...
   --    -- 18. Find the total number of lines in Vertical field period
      TOTAL_V_LINES : Scan_Line_Count := V_LINES_RND + TOP_MARGIN + BOT_MARGIN + Scan_Line_Count (INTERLACE) + V_SYNC_BP + MIN_PORCH_RND;
...
--  6. Find the horizontal addressable time (in chars):
   H_ADDR_TIME_CHARS : Character_Count := Character_Count (Float'Rounding(Float (H_PIXELS_RND) / Float (CELL_GRAN)));

--  8. Find the horizontal blanking time (in chars):
   H_BLANK_CHAR : Character_Count := To_Character_Count (H_BLANK);

--  17. Find the number of pixels in the horizontal sync period:
   H_SYNC_PIXEL : Pixel_Count := Pixel_Count (Float'Rounding( H_SYNC_P / 100.0 * Float (TOTAL_PIXELS) / Float (CELL_GRAN)) * Float (CELL_GRAN));
...
--  20. Find the number of characters in the horizontal sync period:
   ...
   V_EVEN_BLANKING_LINES : Scan_Line_Count := V_SYNC_BP + Scan_Line_Count (2.0 * INTERLACE) + MIN_PORCH_RND;

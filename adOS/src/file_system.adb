...
package body File_System is
   ...
   function Add_File
     (File_System : File_System_Type; File_System_Decriptor : Driver_File_Descriptor_With_Error)
      return File_Descriptor_With_Error is
   begin
      ...
      Logger.Log_Error ("No available file descriptor");
      return FD_ERROR;
   end Add_File;
   ...
   begin
      ...
      for File_System in SERIAL_FS .. PIT_FS loop
         ...
         if Driver_FD /= DRIVER_FD_ERROR then
            ...
            if FD = FD_ERROR then
               close (File_System, Driver_File_Descriptor (Driver_FD));
            end if;
         end if;
   ...
   function close (FS : File_System_Type; Driver_fd : Driver_File_Descriptor) return Integer is
      Result : Integer := -1;
   begin
      case FS is
         when SERIAL_FS =>
         ...
         when ISO_FS =>
            Result := File_System.ISO.close (Driver_fd);

         when VGA_FS =>
            Result := File_System.VGA.close (Driver_fd);

         when PIT_FS =>
            Result := File_System.IRQ.close (Driver_fd);
   ...
   procedure close (FS : File_System_Type; Driver_fd : Driver_File_Descriptor) is
      Result : Integer;
      pragma Unreferenced (Result);
   begin
      Result := close (FS, Driver_fd);
   end close;
   ...
   begin
      ...
      Result := close (File.File_System, File.File_System_Decriptor);
      if Result = 0 then
